...
let findBounds [n][m] (vals: [n]f32) (num_bins: i64) (dest: *[m]f32): [m]f32 =
...
let binMap [n] (vals: [n]f32) (num_bins: i64) : ([n]u16, [num_bins]f32) =
  ...
  let new_bounds = findBounds s_vals num_bins dest
...
entry trans [n][d] (data: [n][d]f32) (_: [n]f32) =
  let b = 256
  in
  binMap_seq (transpose data) b
...
-- compiled input @ ../data.gz
-- input @ ../data/HIGGS_training
entry seq [n][d] (data: [n][d]f32) (_: [n]f32) =
  let b = 256
  in
  binMap_seq_v1 data b
...
-- compiled input @ ../data.gz
-- input @ ../data/HIGGS_training
entry justsort [n][d] (data: [n][d]f32) (_: [n]f32) =
  let data_b = replicate (n*d) 0f32

  let (data_b) =
    loop (data_b) = (data_b) for i < d do
    let data_entry = radix_sort f32.num_bits f32.get_bit data[:, i]
    let offset_d = i*n
    let data_offsets = iota n |> map (+offset_d)
    in
    (scatter data_b data_offsets data_entry)
  in
  unflatten d n data_b |> transpose
